...
class LabPygame(pygame.sprite.Sprite):
    ...
    def button(self, window_surface, position, text, buttoncolor=(120, 120, 120), linecolor=(20, 20, 20), textcolor=TEXT_COLOR, bwidth=200, bheight=50):
    ...
    def interface(self, mode='game_start'):
        ...
        if mode == 'game_start':
            ...
            while True:
                ...
                button_1 = self.button(self.window_surface, (150, 290), 'START')
    ...
    def display_elements(self):
        ...
        for weap in self.maze.weapons:
            if not weap.has_position():
                continue
            if weap.name == "needle":
                self.window_surface.blit(self.needle_render, (weap.x * SPRITE_SIZE, weap.y * SPRITE_SIZE))
            elif weap.name == "ether":
                self.window_surface.blit(self.ether_render, (weap.x * SPRITE_SIZE, weap.y * SPRITE_SIZE))
            elif weap.name == "pipe":
                self.window_surface.blit(self.pipe_render, (weap.x * SPRITE_SIZE, weap.y * SPRITE_SIZE))
...
class Bar(LabPygame, pygame.sprite.Sprite):
    ...
    def show_bar(self):
        if len(self.hero.inventory) == 0:
            ...
            self.image = self.font.render(
                f"You can sedate the guardian and leave the maze!",
                True, TEXT_COLOR)
        return self.image
